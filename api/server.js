...
// bcrypt con fallback texto plano para usuarios legacy + auto-migración
...
// RUTAS BASE
// ══════════════════════════════════════════════════════════════
app.get("/",       (_, res) => res.json({ status: "online", version: "9.0", timestamp: new Date().toISOString() }));
...
// REGISTRO PÚBLICO
// POST /registro
// Body: { nombre_persona, apellido?, email, telefono?,
//         business_name, password, horarios?, duracion_turno? }
// ══════════════════════════════════════════════════════════════
app.post("/registro", limiterAuth, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, slug: nuevo.slug, business_name: nuevo.business_name, panel_url: `${PANEL_URL}?u=${nuevo.slug}`, token, dias_prueba: DIAS_PRUEBA, fecha_vencimiento: fechaVencimiento });
  } catch (e) {
...
app.post("/login", limiterAuth, async (req, res) => {
  try {
    ...
    let query = supabase.from("usuarios")
      .select("id, slug, password, business_name, nombre_persona, apellido, email, activo, estado_suscripcion, fecha_vencimiento");
...
// ══════════════════════════════════════════════════════════════
// ADMIN — RESETEAR PASSWORD (para emergencias via Postman/consola)
...
app.post("/turnos/reservar", limiterBooking, async (req, res) => {
  try {
    ...
    const { data: user, error: userError } = await supabase.from("usuarios").select("*").eq("slug", slugClean).single();
    ...
    if (servicio_id) {
      const { data: srv } = await supabase.from("servicios").select("nombre, capacidad, precio").eq("id", servicio_id).single();
...
app.get("/settings/:slug", requireAuth, async (req, res) => {
  try {
    const slug = cleanSlug(req.params.slug);
    const { data: user, error } = await supabase.from("usuarios")
      .select("slug, business_name, rubro, nombre_persona, apellido, email, telefono, duracion_turno, capacidad_por_turno, metodo_pago, porcentaje_sena, quien_asume_comision, horarios, excepciones, mp_access_token, mobbex_api_key, estado_suscripcion, fecha_vencimiento")
      .eq("slug", slug).single();
    if (error || !user) return res.status(404).json({ success: false, error: "Negocio no encontrado." });
...
const handleUpdateSettings = async (req, res) => {
  try {
    ...
    const { horarios, duracion_turno, excepciones, porcentaje_sena, metodo_pago, quien_asume_comision, capacidad_por_turno, business_name, telefono, rubro } = req.body;
    const u = {};
...
app.get("/admin-stats/:slug", requireAuth, async (req, res) => {
  try {
    ...
    const { data: todosLosTurnos } = await supabase.from("turnos").select("telefono, email, created_at").eq("slug", slug).neq("estado", "cancelado");
...
app.post("/superadmin/negocios", requireAdminKey, async (req, res) => {
  try {
    const { nombre_persona, apellido, email, telefono, business_name, password, rubro = "generico" } = req.body;
    if (!nombre_persona || !email || !password || !business_name) {
    ...
    const { data, error } = await supabase.from("usuarios").insert([{
      ...
      business_name: business_name.trim(), slug, password: hashedPassword, rubro,
    ...
    console.log(`✅ Negocio creado (admin): ${slug} — vence: ${fechaVencimiento}`);
...
app.get("/superadmin/negocios", requireAdminKey, async (req, res) => {
  try {
    ...
    const negocios = (data || []).map((u) => ({
      ...
      activo: u.activo, metodo_pago: u.metodo_pago, tiene_mp: !!u.mp_access_token, tiene_mobbex: !!u.mobbex_api_key,
...
app.put("/superadmin/negocios/:slug", requireAdminKey, async (req, res) => {
  try {
    const slug    = cleanSlug(req.params.slug);
    const allowed = ["nombre_persona", "apellido", "email", "telefono", "business_name", "rubro", "activo", "duracion_turno", "capacidad_por_turno", "estado_suscripcion", "fecha_vencimiento"];
    const update  = {};
...
app.post("/api/create-preference", limiterBooking, async (req, res) => {
  try {
    ...
    if (servicio_id) {
      const { data: srv } = await supabase.from("servicios").select("nombre, precio").eq("id", servicio_id).eq("slug", slugClean).single();
    ...
    if (user.mp_access_token) {
      try {
        ...
        const response = await pref.create({ body: { items: [{ title: `${nombreServicio} (${conceptoPago}): ${fecha} - ${hora}hs`, unit_price: montoACobrar, quantity: 1, currency_id: "ARS" }], metadata: { ...metaMeta, tipo_pago: metodo }, notification_url: `${API_URL}/webhook/mp`, back_urls: { success: successUrl, failure: cancelUrl, pending: cancelUrl }, auto_return: "approved" } });
...
app.post("/api/suscripcion/checkout", requireAuth, async (req, res) => {
  try {
    ...
    const { data: user, error } = await supabase.from("usuarios")
      .select("id, email, nombre_persona, apellido, business_name, fecha_vencimiento")
      .eq("slug", slug).single();
    if (error || !user) return res.status(404).json({ success: false, error: "Negocio no encontrado." });
...
app.get("/api/suscripcion/estado", requireAuth, async (req, res) => {
  try {
    ...
    const { data: user, error } = await supabase.from("usuarios").select("estado_suscripcion, fecha_vencimiento").eq("slug", slug).single();
    if (error || !user) return res.status(404).json({ success: false, error: "Negocio no encontrado." });
...
async function procesarPagoSuscripcion(payData) {
  ...
  const { data: user, error } = await supabase.from("usuarios").select("id, email, nombre_persona, fecha_vencimiento").eq("slug", slug).single();
  ...
  await supabase.from("usuarios").update({ fecha_vencimiento: nuevaFecha, estado_suscripcion: "activo" }).eq("slug", slug);
  invalidateCache(slug);
